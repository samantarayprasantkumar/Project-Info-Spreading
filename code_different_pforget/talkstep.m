%Script letting people meet, eventually meet and talk, updates status
...
for i=1:N
   ...
   if(check(p1)==0 && check(p2)==0 && p1~=p2)
      ...
      if((status(p1)+status(p2))==1)
          ...
          pinform=(common(p1,p2)+1)/(maxcommon+1); %Patrick is also a common friend :D
...
% %          if(rand<pforget) %probability that they forget, normal simulation
         if(rand<pforget(NR_PFROGET)) %probability that they forget
...
% %          if(rand<pforget)  %only by a certain probability, normal simulation
         if(rand<pforget(NR_PFROGET))  %only by a certain probability
